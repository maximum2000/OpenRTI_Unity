...
extern "C"
{
    typedef void(*сonnectionLostCallback)(const char* message, int size);
    static сonnectionLostCallback сonnectionLostCallbackFunction = nullptr;
    __declspec(dllexport) void RegisterConnectionLostCallback(сonnectionLostCallback callback);
}
...
void RegisterConnectionLostCallback(сonnectionLostCallback callback)
{
    сonnectionLostCallbackFunction = callback;
}
...
namespace OpenRTI
{
    ...
    public:
        ...
        //запрос списка федератов в федерации, ответ см. reportFederationExecutions
...
//!!!!!!!!!!!!!!!!
    ...
    protected:
            ...
            RTI_THROW((rti1516e::FederateInternalError))
        {
            ...
            const char* tmsg = s.c_str();
            if (сonnectionLostCallbackFunction != nullptr)
            {
                сonnectionLostCallbackFunction(tmsg, (int)strlen(tmsg));
            }

        }

        //callback после вызова ListFederationExecutions ... получен список федератов
        ...
        //сообщение о удачной попытке регистрации точки синхронизации 
        ...
        //сообщение о неудачной попытке регистрации точки синхронизации 
        ...
        //сообщение что один из федератов установил точку синхронизации
        ...
        //сообщение что федерация синхронизирована
